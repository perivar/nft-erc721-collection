...
import '@openzeppelin/hardhat-upgrades'; // added by PIN to support upgradeable contracts in tests
...
task('generate-root-hash', 'Generates and prints out the root hash for the current whitelist', async () => {
  ...
  if (CollectionConfig.whitelistAddresses.length < 1) {
    throw new Error('The whitelist is empty, please add some addresses to the configuration.');
  }
...
task(
  ...
  async (taskArgs: { newName: string }, hre) => {
    ...
    if (!fs.existsSync(oldContractFile)) {
      throw new Error(`Contract file not found: "${oldContractFile}" (did you change the configuration manually?)`);
    }

    if (fs.existsSync(newContractFile)) {
      throw new Error(`A file with that name already exists: "${oldContractFile}"`);
    }
